		...
		for ($i=0; $i < $cnt; $i++) {
			...
			if ($s_month < 0) {		//  Сб./Вс. перед/после праздника или Праздник в Сб./Вс. перед/после даты 
				...
				if (($f_e+7 == $ep_e) && ($s_month == -1 && $s_date == 0 && $f_month == 1 && $f_date == 6 && ($type == 204 || $type == 207))) {
					...
					if ($s_month == -1 && $we == $s_date) $name = "";												// Если Сб./Вс. приходится на самый день праздника, то не отмечается
					if ($s_month == -3 && $we != $s_date) $name = "";												// Если праздник не совпадает с указанным днем недели, то не отмечается
					if ($y == 0) { $i--; $y = 1;} 																	// Проверяем дважды: для текущего и следующего года
					else {$y = 0;}
				}
			}
			else {
				if($s_month > 0) $start =  date( 'U', mktime ( 0, 0, 0, $s_month, $s_date, $year ) );			// Неподвижные события - начало периода
				else if ($s_month == 0) {																		// Переходящие события - начало периода
					if ($type==202) $start = shift202 ($s_date, $date, $year);										// Чтения на утрени
					else if ($type==204) $start = shift204 ($s_date, $date, $year);		  							// Апостол на Литургии
				...
				if ($f_month > 0) $finish =  date( 'U', mktime ( 0, 0, 0, $f_month, $f_date, $year ) );			// Неподвижные события - конец периода
				else if ($f_month == 0) {																		// Переходящие события - конец периода
					if ($type==202) $finish = shift202 ($s_date, $date, $year);										// Чтения на утрени
					else if ($type==204) $finish = shift204 ($s_date, $date, $year);								// Апостол на Литургии
...
//	if (!checkDays ($d, $year)) return false;					// Проверка особых дней
...
function shift207 ($d, $date, $year) {				// Смещение даты для Евангелие на Литургии
	...
	if ($d == 245) $d = $ff_e;									// Неделя 28-я
    else if ($d == $ff_e) $d = 245;
...
//	if (!checkDays ($d, $year)) return false;					// Проверка особых дней
	...
	if ($d > 168) $d = $d+($er_e-168);							// Смещение на величину Воздвиженской отступки $er_e-168
	if ($date > $easter+168*(3600*24) && $date <= $easter+$er_e*(3600*24)) { 	// Если 18-я седмица случилась до Недели по Воздвижении, 
		if ($d > 168-($er_e-168) && $d <= 168) $d = $d+($er_e-168);				// то дублируем чтения предыдущих(ей) седмиц(ы)
	}
...
*******************************************************************************/  
function checkDays ($d, $year) {
	$easter = easter('U', $year, true);							// Пасха в текущем году
	$easter_prev = easter('U', $year-1, true);					// Пасха в предыдущем году
	if ($d < 0) $d = $d + ($easter - $easter_prev)/(3600*24);	// Отсчет только от предыдущей Пасхи

	$wd = date( 'w', mktime ( 0, 0, 0, 12, 25+dd($year-1), $year-1 ));
	$f_e = (date( 'U', mktime ( 0, 0, 0,12, 25, $year-1 ))-$easter_prev)/(3600*24)-$wd; // Кол-во дней до Недели св.отцов от Пасхи
	if ($d == $f_e-1) return false; 				// Суббота перед РХ
	if ($d == $f_e && $wd !=0) return false; 		// Неделя Св. Отцов
	if ($d == $f_e+6 && $wd !=6) return false;		// Суббота по РХ
	if ($d == $f_e+7) return false;					// Неделя Богоотцов

	$wd = date( 'w', mktime ( 0, 0, 0, 1, 6+dd($year), $year ));
	$ep_e = (date( 'U', mktime ( 0, 0, 0, 1, 6, $year ))-$easter_prev)/(3600*24)-$wd; // Кол-во дней до Недели перед Богоявлением от Пасхи
	if ($d == $ep_e-1) return false;				// Суббота перед Богоявлением
	if ($f_e+7 == $ep_e && $d == (date( 'U', mktime ( 0, 0, 0, 1, 1, $year ))-$easter_prev)/(3600*24)) return false;		// Если неделя Богоотцов совпадает с Неделей перед Богоявлением, то чтения Недели перед Богоявлением переносятся на 1 января ст.ст.
	else if ($d == $ep_e && $wd !=0) return false;	// Неделя перед Богоявлением
	if ($d == $ep_e+6 && $wd !=6) return false;		// Суббота по Богоявлению
	if ($d == $ep_e+7) return false;				// Неделя по Богоявлению
	return true;
}
/*******************************************************************************
...
function showDayInfo ( $day,				// День (по умолчанию - сегодня)
					...
					$sedmica,				// Седмица
					$memory,				// Памятные дни и дни поминовения усопших
					...
					$links )				// Ссылки и цитаты
{
	...
	if (substr ($day,0,1) == '-') {			// Смещение текущей даты назад
		$year = date('Y');
		$month = date('m');
	...
	if ($cnt) {
		...
		if ($readings != 'off') {
			$q ="";
			for ($id=0; $id < 100; $id++) {$qq[$id]="";}
			$qtitle = '';
			for ($i=0; $i < $cnt; $i++) {
				if ($e[$i]['type'] >= 200 && $e[$i]['type'] <= 299) {							
					$id = $e[$i]['type']-200;
					$qq[$id] .= (($qq[$id]!="")?('; '):('')).$e[$i]['name'];
				}
			}
...
// В праздники Рождества Христова и Богоявления Господня, случившиеся в воскресенье, рядовое воскресное зачало не читается. 
				...
				if ($q8 == "" && $q1 == "" && !(($q2 != "" || $q3 != "")&& $wd != 0)) $qz .= (($qz!="")?('<br>Ряд.: '):('')).printReadins ($qq, 0);	// Рядовые чтения
				if ($q2 != "" || $q3 != "") $qz .= (($qz!="")?('<br>'):(''))."<em><strong>Праздник:</strong></em> ".$q2." ".$q3;
				if ($q4 != "" || $q5 != "" || $q6 != "") $qz .= (($qz!="")?('<br>'):(''))."<em>Праздник:</em> ".$q4." ".$q5." ".$q6;
				$q .= $qz;
				if ($readings!='on') $qtitle = htmlspecialchars_decode($readings);
			}

			if (function_exists ('bg_bibfers_bible_proc') && $links != 'off') $q = bg_bibfers_bible_proc($q, $links);
